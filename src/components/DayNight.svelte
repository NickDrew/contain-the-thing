<script>
    export let dayNightDuration;
    let startDateTime = Date.now();
    let paused = true;
    let currentTimeRemaining = dayNightDuration;
    let isDay = true;

    function buildTimerFromMilli(milli) {
    ...
    setInterval(() => {
        if (!paused) {
            ...
            currentTimeRemaining =
                dayNightDuration - (currentDateTime - startDateTime);
    ...
    function togglePause() {
        ...
        if (!paused) {
            const currentDateTime = Date.now();
            startDateTime =
                currentDateTime + currentTimeRemaining - dayNightDuration;
        }
    }
    ...
    function toggleIsDay() {
        ...
        currentTimeRemaining = dayNightDuration;
    }
</script>
...
<div class="day-night-outer">
    <div class="crt" />
    <div>
        {#if isDay}Day{:else}Night{/if}
        Remaining
    </div>
    <div>{buildTimerFromMilli(currentTimeRemaining)}</div>
    <button
        on:click={() => {
            togglePause();
        }}>
        {#if paused}Unpause{:else}Pause{/if}
    </button>
    <button on:click={() => toggleIsDay()}> Skip </button>
</div>
